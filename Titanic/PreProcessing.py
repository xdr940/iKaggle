...
def preprocess(trainPath,testPath):
	...
	df_train = df_train[['Survived','Pclass','Sex','Age','Fare']]#只用这五列
	df_test = df_test[['Pclass','Sex','Age','Fare']]#只用这五列

	df_train['Sex']=df_train['Sex'].replace(['male','female'],[1,0])#把male换成1，female换成0
	df_test['Sex']=df_test['Sex'].replace(['male','female'],[1,0])#把male换成1，female换成0

	df_train=df_train.fillna(0)#空值添零处理
